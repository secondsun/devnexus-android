...
public class CachingImageProvider {
    ...
    private CachingImageProvider(Context ctx) {
        cacheDir = getDiskCacheDir(ctx, IMAGE_DIR);
        if (!cacheDir.exists()) {
            cacheDir.mkdir();
        }
    }

    public static CachingImageProvider getInstance(Context ctx) {
        if (instance == null) {
            return instance = new CachingImageProvider(ctx.getApplicationContext());
        }
        return instance;
    }
