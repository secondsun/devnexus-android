...
public class CenteringDecoration extends RecyclerView.ItemDecoration {
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        int originalLeft = (parent.getMeasuredWidth() / spanCount) * columnPosition;
        int originalRight = (parent.getMeasuredWidth() / spanCount) * (columnPosition + 2);
        if (rowItemsCount == 1) {
            originalLeft = 0;
            originalRight = parent.getMeasuredWidth();
        }

        int newFarLeft = (parent.getMeasuredWidth()  - viewWidth * rowItemsCount) / 2;
